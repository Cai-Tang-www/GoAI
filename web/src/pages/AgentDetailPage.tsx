...
export function AgentDetailPage() {
  ...
  const refresh = async () => { await client.invalidateQueries({ queryKey: ['agent', agentCode] }); await client.invalidateQueries({ queryKey: ['workflows', agentCode] }) }
  ...
  const submit = async (values: Record<string, unknown>) => {
    setSubmitting(true)
    try {
      if (modal === 'agent') await apiRequest(`/api/agents/${encodeURIComponent(agentCode)}`, { method: 'PUT', body: values })
      if (modal === 'capability') {
  ...
  const action = async (path: string, success: string) => {
    try { await apiRequest(path, { method: 'POST' }); message.success(success); await refresh() } catch (error) { message.error(errorDescription(error)) }
  }
  ...
  const tabs = [
    ...
    { key: 'endpoints', label: `Endpoints ${endpoints.length}`, children: <section className="content-section"><div className="section-heading"><div><h2>A2A Endpoints</h2><span>健康检查通过后才能参与发布</span></div><Button type="primary" icon={<Plus size={15} />} onClick={() => openModal('endpoint')}>新建</Button></div><Table rowKey="endpoint_code" dataSource={endpoints} scroll={{ x: 900 }} pagination={false} columns={[{ title: 'Endpoint', render: (_, item) => <div className="table-primary"><span><strong>{item.endpoint_code}</strong><code>{item.transport} · {item.protocol}</code></span></div> }, { title: '地址', dataIndex: 'address', ellipsis: true, render: (value) => <code>{value}</code> }, { title: '健康状态', render: (_, item) => <div><StatusTag status={item.status} /><small className="table-note">{item.last_healthy_at ? formatTime(item.last_healthy_at) : '尚未检查'}</small></div> }, { title: '操作', width: 190, render: (_, item) => <div className="row-actions"><Button size="small" icon={<HeartPulse size={14} />} onClick={() => action(`/api/agents/${encodeURIComponent(agentCode)}/endpoints/${encodeURIComponent(item.endpoint_code)}/health-check`, '健康检查通过')}>健康检查</Button><Button size="small" onClick={() => openModal('endpoint', item)}>编辑</Button></div> }]} /></section> },
