...
import { Alert, Button, Descriptions, Form, Input, Modal, Popconfirm, Select, Table, Tabs, message } from 'antd'
...
export function AgentDetailPage() {
  ...
  const [modal, setModal] = useState<'agent' | 'capability' | 'endpoint' | null>(null)
  ...
  const tabs = [
    ...
    { key: 'workflows', label: `Workflows ${workflows.length}`, children: <section className="content-section"><div className="section-heading"><div><h2>Workflow 版本</h2><span>active 版本不可原地编辑</span></div><Button type="primary" icon={<Plus size={15} />} onClick={() => navigate(`/agents/${agentCode}/workflows/new`)}>新建版本</Button></div><Table rowKey="version" loading={workflowsQuery.isLoading} dataSource={workflows} pagination={false} columns={[{ title: '版本', render: (_, item) => <Link to={`/agents/${agentCode}/workflows/${item.version}`}><strong>v{item.version}</strong></Link> }, { title: 'Checksum', dataIndex: 'checksum', render: (value) => <code>{shortId(value, 16)}</code> }, { title: '状态', dataIndex: 'is_active', render: (value) => <StatusTag status={value ? 'active' : 'inactive'} /> }, { title: 'Capability 引用', dataIndex: 'capabilities', render: (values) => values?.length ? values.map((value: { capability_code: string }) => <code key={value.capability_code}>{value.capability_code} </code>) : '—' }, { title: '操作', render: (_, item) => <div className="row-actions"><Link to={`/agents/${agentCode}/workflows/${item.version}`}><Button size="small">查看</Button></Link>{item.is_active ? <><Link to={`/agents/${agentCode}/workflows/new?from=${item.version}`}><Button size="small">基于此新建</Button></Link><Popconfirm title="确认停用此版本？" onConfirm={() => action(`/api/agents/${encodeURIComponent(agentCode)}/workflows/${item.version}/deactivate`, 'Workflow 已停用')}><Button size="small">停用</Button></Popconfirm></> : <><Link to={`/agents/${agentCode}/workflows/${item.version}/edit`}><Button size="small">编辑</Button></Link><Button size="small" type="primary" onClick={() => action(`/api/agents/${encodeURIComponent(agentCode)}/workflows/${item.version}/activate`, 'Workflow 已激活')}>激活</Button></> }</div> }]} /></section> },
  ...
  return (
    <div className="page-shell">
      ...
      <Modal title={modal === 'agent' ? '编辑 Agent' : modal === 'capability' ? `${editing ? '编辑' : '新建'} Capability` : `${editing ? '编辑' : '新建'} Endpoint`} width={620} open={Boolean(modal)} onCancel={() => setModal(null)} onOk={() => form.submit()} confirmLoading={submitting} okText="保存">
