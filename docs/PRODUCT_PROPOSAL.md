...
- Workflow 的 `agent_tool` 节点已实现为 Eino `InvokableTool` 适配器；Tool 只改变调用方的 Graph 语义，底层仍通过 Registry、A2A Client 和目标 Agent Gateway 通信，并复用 Delegation、Child Run、callback/resume 和输入输出契约校验
